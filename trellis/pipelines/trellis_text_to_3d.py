...
class TrellisTextTo3DPipeline(Pipeline):
    ...
    def voxelize(self, mesh: trimesh.Trimesh) -> torch.Tensor:
        """
        Voxelize a mesh using Trimesh.
        """
        vertices = mesh.vertices - mesh.centroid
        vertices = vertices / mesh.scale
        vertices = np.clip(vertices, -0.5 + 1e-6, 0.5 - 1e-6)
        normalized_mesh = trimesh.Trimesh(vertices=vertices, faces=mesh.faces)

        voxel_grid = normalized_mesh.voxelized(pitch=1.0/64)

        vertices = voxel_grid.indices + 32 
