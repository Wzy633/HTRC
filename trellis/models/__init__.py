...
def from_pretrained(path: str, **kwargs):
    ...
    config_file = Path(f"{prefix}.json")
    model_file = Path(f"{prefix}.safetensors")
    missing = [str(file) for file in (config_file, model_file) if not file.is_file()]
    if missing:
        raise FileNotFoundError(
            "local TRELLIS checkpoint is incomplete; network fallback is disabled. "
            f"Missing: {missing}")
